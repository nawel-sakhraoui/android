      ...
      .subscribe (
          ...
          data =>{
             ...
             //    console.log( JSON.parse(this.model[i]._source.messages[j]) );
                  ...
                  this.model[i].articlesrating = {} ; 
                 /*    this.messagesService.getOngoingMessages(this.model[i]._id)
                  .subscribe(
                   message => 
                   {
                       console.log(message) ;
  ...
  sendRating (i, storetitle) { 
         let bool = true ; 
            ...
            for (let a of this.model[i]._source.articles){
             if (!(a.articleid in this.model[i].articlesrating) || this.model[i].articlesrating[a.articleid] ==0 )
             {    
